 ...
 */
class Mage_Payco_Model_Api_Gateway_Fps extends Mage_Payco_Model_Api_Gateway_Abstract
{
    /*
     ...
     * @return Mage_Payco_Model_Api_Gateway_Fps_Request_Abstract
     */
    public function getRequest($actionCode)
    {
        switch ($actionCode) {
            case self::ACTION_CODE_CANCEL: 
                $requestModelPath = 'Payco/api_gateway_fps_request_cancel';
                break;  
            case self::ACTION_CODE_SETTLE:
                $requestModelPath = 'Payco/api_gateway_fps_request_settle';
                break;  
            case self::ACTION_CODE_REFUND:
                $requestModelPath = 'Payco/api_gateway_fps_request_refund';
                break;  
    ...
    protected function _getResponse($requestActionCode, $responseBody)
    {
        switch ($requestActionCode) {
            case self::ACTION_CODE_CANCEL: 
                $responseModelPath = 'Payco/api_gateway_fps_response_cancel';
                break;  
            case self::ACTION_CODE_SETTLE:
                $responseModelPath = 'Payco/api_gateway_fps_response_settle';
                break;  
            case self::ACTION_CODE_REFUND:
                $responseModelPath = 'Payco/api_gateway_fps_response_refund';
                break;  
            default: $this->_throwExeptionInvalidActionCode();
        }
        ...
        $errorResponse = Mage::getSingleton('Payco/api_gateway_fps_response_error');
        if ($errorResponse->init($responseBody)) {
            return $errorResponse;
        }

        throw new Exception(
            Mage::helper('Payco')->__('Response body is not valid FPS respons'), 
     ...
     * Process FPS request
     *
     * @param object Mage_Payco_Model_Api_Gateway_Fps_Request_Abstract $request
     * @return Mage_Payco_Model_Api_Gateway_Fps_Response_Abstract
     */
    public function process($request)
    {
        if (!$request->isValid()) {
            throw new Exception(
                Mage::helper('Payco')->__('Invalid request'), 
     ...
     *
     * @param object Mage_Payco_Model_Api_Gateway_Fps_Request_Abstract
     * @return Mage_Payco_Model_Api_Gateway_Fps_Request_Abstract
     */
    ...
    {
        throw new Exception(
            Mage::helper('Payco')->__('Invalid action code'), 
