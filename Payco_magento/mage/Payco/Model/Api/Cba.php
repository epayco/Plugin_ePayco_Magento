...
/**
 * Payco CBA API wrappers model
 *
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Mage_Payco_Model_Api_Cba extends Mage_Payco_Model_Api_Abstract
{
    protected static $HMAC_SHA1_ALGORITHM = "sha1";
    protected $_paymentCode = 'Payco_cba';
    ...
    {
        return Mage::getStoreConfig('payment/Payco_cba/merchant_id');
    }
    ...
    public function getAmazonRedirectUrl()
    {
        ...
        $_merchantId = Mage::getStoreConfig('payment/Payco_cba/merchant_id');
    ...
    public function getConfigShippingRates()
    {
        if (is_null($this->_configShippingRates)) {
            ...
            foreach ($this->_shippingRates as $_rate) {
                $_carrier = unserialize(Mage::getStoreConfig('payment/Payco_cba/' . strtolower($_rate) . '_rate'));
    ...
    {
        return Mage::helper('Payco')->formatAmount($amount);
    }
    ...
    public function getXmlCart(Mage_Sales_Model_Quote $quote)
    {
        ...
        $_xml .= " <OrderCalculationCallbacks>\n"
                ...
                ."   <OrderCallbackEndpoint>".Mage::getUrl('Payco/cba/callback', array('_secure' => true))."</OrderCallbackEndpoint>\n"
...
XML;
     ...
     *
     * @return Mage_Payco_Model_Api_Cba_Document
     */
    public function getDocumentApi()
    {
        if (is_null($this->getData('document_api'))) {
            $_documentApi = Mage::getModel('Payco/api_cba_document')
                ->setWsdlUri('https://merchant-api.amazon.com/gateway/merchant-interface-mime/')
                ->setMerchantInfo(array(
                    'merchantIdentifier' => Mage::getStoreConfig('payment/Payco_cba/merchant_tocken'),
                    'merchantName' => Mage::getStoreConfig('payment/Payco_cba/merchant_name'),
                ))
                ->init(
                    Mage::getStoreConfig('payment/Payco_cba/merchant_login'),
                    Mage::getStoreConfig('payment/Payco_cba/merchant_pass')
                );
     ...
     * @return Mage_Payco_Model_Api_Cba
     */
     ...
     * @param Mage_Payco_Model_Api_Cba
     */
